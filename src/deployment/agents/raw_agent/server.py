...
@app.route('/raw_agent', methods=['POST'])
def get():
    rsp = server_pack('raw_agent', raw_agent_api, request.json)
    return rsp

def raw_agent_api(sample):
    ...
    payload = {
        "model": 'b330k-e29k-sft-s-0407-rlhf',
        "messages": messages,
    ...
    try:
        ...
        new_messages = '[-] raw_agent service exception'
...
log_config(args, 'raw_agent_server')
